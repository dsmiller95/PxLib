...
public interface IHavePxShaderMaterial
{
    /// <summary>
...
{
    public IHavePxShaderMaterial MaterialOwner => (_materialOwner as Object != null) ? _materialOwner : (_materialOwner = GetComponent<IHavePxShaderMaterial>());
    private IHavePxShaderMaterial _materialOwner;
    ...
    //Control properties
    private bool _spriteUpdatesFrozen;

    public bool SpriteUpdatesFrozen
    {
        get => _spriteUpdatesFrozen;
        set
        {
            _spriteUpdatesFrozen = value;
            UpdateSprite();
        }
    }
    ...
    // put in sprite shader:
    private void SetUvData()
    {
        var sprite = MaterialOwner.GetSprite();
        if (sprite)
        {
            var dimensions = new Vector2(sprite.texture.width, sprite.texture.height);
    ...
    {
        if(_spriteUpdatesFrozen) return;
        SetUvData();
        MaterialOwner.UpdateMaterial(PropertyBlock =>
        {
